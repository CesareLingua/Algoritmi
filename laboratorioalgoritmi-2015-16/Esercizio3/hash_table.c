...
int delete(node** hash_table, key* k, CompareFunc compare){
  int hash_val;
  hash_val = hash(k->id);
  hash_table[hash_val] = delete_node(hash_table[hash_val], k, compare);
  return 1;
}
...
double delete_key(node** hash_table, char* argv){
  ...
  for(i = 0; i < N_OPER; i++){
    ...
    if(strcmp(argv, "-s") == 0){
      start = clock();
      if(delete(hash_table, k[i], compare_string) == 1)
        del++;
      else
        n_del++; 
      end = clock();
      ...
      start = clock();
      if(delete(hash_table, k[i], compare_long_int) == 1)
        del++;
      else
        n_del++; 
      end = clock();
      ...
      start = clock();
       if(delete(hash_table, k[i], compare_double) == 1)
        del++;
      else
        n_del++;
      end = clock();
  ...
  printf("Cancellazioni completate con successo: %d\n",del);
  printf("Cancellazioni fallite: %d\n",n_del);
  free(k);
  return t;
